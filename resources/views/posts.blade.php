        ...
        <div class="card-body text-center">
            ...
            <p>
                <small class="text-body-secondary text-muted">
                    <p>Diposting oleh <a href="/posts?author={{ $posts[0]->author->username }}" class="text-decoration-none">{{ $posts[0]->author->name }}</a> di <a href="/posts?category={{ $posts[0]->category->slug }}" class="text-decoration-none">{{ $posts[0]->category->name }}</a> pada {{ $posts[0]->created_at->diffForHumans() }}</p>
                </small>
            </p>
            ...
            @foreach ($posts->skip(1) as $post)
                <div class="col-md-4 mb-3">
                    <div class="card">
                        ...
                        <p>
                        <small class="text-body-secondary text-muted">
                            <p>Dari <a href="/posts?author={{ $post->author->username }}" class="text-decoration-none">{{ $post->author->name }}</a> di {{ $post->created_at->diffForHumans() }}</p>
                        </small>
                        </p>
